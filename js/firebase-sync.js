...
const CloudDB = {
  ...
  async seedIfEmpty() {
    ...
    try {
      ...
      if (prodSnap.empty && typeof SEED_PRODUCTS !== 'undefined' && SEED_PRODUCTS.length > 0) {
        console.log('CloudDB: Seeding products (independent check)...');
  ...
  async bootstrap() {
    ...
    try {
      ...
      if (ver < 15) {
        console.log(`CloudDB: Version ${ver} detected. Cleaning up dummy products...`);
        const products = Store.getProducts();
        const dummyIds = Array.from({length: 16}, (_, i) => `prd-${i+1}`);
        const filtered = products.filter(p => !dummyIds.includes(String(p.id)));
