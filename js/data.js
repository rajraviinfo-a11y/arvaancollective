  ...
  init() {
    try {
      ...
      if (!this.get('products') || this.get('products').length === 0) {
        console.log('Store: Local products empty, seeding from SEED_PRODUCTS...');
        this.setProducts(SEED_PRODUCTS || []);
      }

      if (!this.get('sellers') || this.get('sellers').length === 0) {
        console.log('Store: Local sellers empty, seeding default seller...');
        this.setSellers([
          { id: 'seller-default', name: 'Arvaan Boutique', email: 'seller@email.com', password: 'seller123', shopName: 'Arvaan Boutique', phone: '9876543210', address: 'Mumbai', payoutBalance: 50000, joinDate: new Date().toISOString() }
        ]);
      }
